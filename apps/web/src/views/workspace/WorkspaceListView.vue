...
import { createWorkspace, deleteWorkspace, listWorkspaces, updateWorkspace, type Workspace } from '@/api/workspace'
...
async function handleCreateWorkspace(payload: { name: string; description: string; public: boolean }) {
  ...
  try {
    ...
    currentPage.value = 1
     ...
     createErrorMessage.value = error instanceof ApiError ? error.message : t('workspace.createError')
  } finally {
...
async function handleRenameWorkspace(payload: { name: string }) {
  ...
  try {
    ...
    renameErrorMessage.value = error instanceof ApiError ? error.message : t('workspace.renameError')
  } finally {
...
<template>
  <section class="space-y-5">
      ...
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p class="text-[11px] font-mono uppercase tracking-[0.2em] text-base-content/45">Workspace / Directory</p>
            <h2 class="heading-serif mt-2 text-3xl font-bold text-base-content">{{ t('workspace.title') }}</h2>
            <p class="mt-2 max-w-2xl text-sm leading-7 text-base-content/62">{{ t('workspace.subtitle') }}</p>
        </div>
          <button type="button" class="btn btn-primary rounded-sm" @click="openCreateModal">{{ t('workspace.createBtn') }}</button>
      </div>
    ...
    <p v-if="actionError" class="rounded-sm border border-error/30 bg-error/10 px-4 py-3 text-sm text-error">
      {{ actionError }}
    </p>
    ...
    <section v-else-if="viewMode === 'card'" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      >
        ...
        <div class="mt-3 grid grid-cols-2 gap-2 text-xs">
             <div class="rounded-sm bg-base-200/70 px-3 py-2 text-base-content/70">{{ t('workspace.memberCount', { count: item.members }) }}</div>
             <div class="rounded-sm bg-base-200/70 px-3 py-2 text-base-content/70">{{ t('workspace.docCount', { count: item.docs }) }}</div>
        </div>
        ...
        <table class="table table-zebra">
          ...
          <tbody>
            ...
            <tr v-if="!pagedWorkspaces.length">
                <td colspan="7" class="py-10 text-center text-sm text-base-content/60">{{ t('workspace.noResults') }}</td>
            </tr>
          </tbody>
        </table>
      </div>
